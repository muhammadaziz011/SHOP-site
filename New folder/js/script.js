...
/* ================= STATE ================= */
const state = {
  cart: [],            // {productId, variant, colorName, qty}
  wishlist: new Set(),
  compare: [],          // productIds
  view: 'home',
...
function removeCartItem(idx){ state.cart.splice(idx,1); renderCart(); }
function renderCart(){
  ...
  document.getElementById('cartFoot').innerHTML = `
    ...
    <button class="btn btn-primary btn-block" onclick="checkoutDemo()">Buyurtmani rasmiylashtirish</button>
...
function checkoutDemo(){ showToast("Bu demo versiya — buyurtma rasmiylashtirish hali ulanmagan."); }
...
function clearCompare(){ state.compare=[]; renderCompareBar(); if(state.view==='listing') renderListingGrid(); if(state.view==='home') renderHome(); }
